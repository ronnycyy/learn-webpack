...
For more information, see https://webpack.js.org/api/cli/.`);
	...
	yargs.parse(process.argv.slice(2), (err, argv, output) => {
		...
		// options来源:
		// 1. webpack.config.js 的 module.exports 对象
		// 2. webpack --entry 等命令提供的数据
		let options;
		try {
			// 综合 --config 指定的 webpack.config.js 还有  webpack --xx (..argv) --entry啊 --output啊 这种命令式配置
			// 合并成一套配置，返回给 options
		...
		function processOptions(options) {
			...
			if (argv.progress) {
				// 命令上有 progress，那就加载内置插件 ProgressPlugin
			...
			if (firstOptions.watch || options.watch) {
				// 以 --watch 方式运行，最终执行 compiler.watch 方法
				const watchOptions =
